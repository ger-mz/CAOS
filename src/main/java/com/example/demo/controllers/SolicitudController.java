...
public class SolicitudController {
    ...
    public ResponseEntity<?> actualizarSolicitud(@RequestBody EstadoSolicitud estadoSolicitud){
        if(estadoSolicitud.getId() == null){
            return ResponseEntity.badRequest().body(new ErrorMessage("Se requiere id de solicitud para actualizar la solicitud"));
        }

        EstadoSolicitud response = maquinaEstadosService.actualizarSolicitud(estadoSolicitud);

        if(response == null){
            ResponseEntity.internalServerError().body(new ErrorMessage("Servicio no disponible"));
        }
    ...
    public ResponseEntity<?> validarEstadoActual(@RequestParam Long id){
        ...
        if(solicitud.getState() == StatusStateEnum.CREADO){
            ...
            return ResponseEntity.badRequest().body(new ErrorMessage("Paquete recibido en mal estado, se realiza devolucion"));
        }
        ...
        if(response.getState() == StatusStateEnum.CANCELACION){
            return ResponseEntity.badRequest().body(new ErrorMessage("Fecha de entrega estimada vencida"));
        }

        if(response.getState() == StatusStateEnum.DEVOLUCION){
            return ResponseEntity.badRequest().body(new ErrorMessage("En proceso de devolucion de paquete"));
        }
